---
title: "Figure 2: Tweedie distribution"
output: pdf_document
---

### Figure 2, New et al: Plotting the Tweedie figures for the gene and species analyses

####First, species abundances
```{r}
abund <- read.table("/workdir/users/fnn3/twins_uk/taxa/abundance/twinsUK_species_abundance_gmeanNormalized_v2.txt", sep="\t")
...
meanvar_coefs <- summary(lm(log.var2~log.mean.abund2))$coefficients[,1] #7.359187        1.538918 
...
nb_meanvar_coefs <- c(log(1), summary(lm(log.var2~0+log.mean.abund2))$coefficients[,1]) # 0, 2.12
...
pdf("/workdir/users/fnn3/twins_uk/scca/plots/tweedie_transformation_mean_plot_species_full.pdf", height=5, width=10)
...
pdf("/workdir/users/fnn3/twins_uk/scca/plots/tweedie_mean_vs_var_plot_w_poisson_nb_species_full.pdf", height=5, width=10)
ggplot(data.frame('x'=log.mean.abund2, 'y'=log.var2, 'col'=pofzeros.abund2), aes(x=x, y=y, colour=col))+
  ...
  geom_abline(intercept=10, slope=1.79, col='#FFB900') +  # from tweedie mean and var
...
ggplot(data.frame('x'=log.mean.abund2, 'y'=log.var2, 'col'=pofzeros.abund2), aes(x=x, y=y, colour=col))+
  ...
  xlab("log(mean abundance)") + ylab("log(variance)") +
  geom_abline(intercept=0, slope=1, col='#9B9B9B') + # poisson
  geom_abline(intercept=meanvar_coefs[1], slope=1.75, col='#FFB900') +  # from tweedie mean and var
  geom_abline(intercept=nb_meanvar_coefs[1], slope=nb_meanvar_coefs[2], col='#484848') +  # from NB1 mean and var
  theme_classic()
dev.off()
```