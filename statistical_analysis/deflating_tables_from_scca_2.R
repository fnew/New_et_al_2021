...
beta <- read.table("/workdir/users/fnn3/twins_uk/scca/gene_results/twinsUK_beta_results_mod.csv", sep="\t", header=T)
...
rownames(snp) <- snp[,1]
snp[,1] <- NULL
spec <- as.data.frame(fread("/workdir/users/fnn3/twins_uk/scca/species/species_abundance_residuals_mod.csv", header=T, sep='\t'), row.names=1)
rownames(spec) <- spec[,1]
spec[,1] <- NULL

alphaS <- read.csv("/workdir/users/fnn3/twins_uk/scca/species/twinsUK_species_alpha_results.csv")
betaS <- read.csv("/workdir/users/fnn3/twins_uk/scca/species/twinsUK_species_beta_results.csv", header=T)
...
write.csv(snp.deflatedS, "/workdir/users/fnn3/twins_uk/scca/species/twins240_deflatedSNPs_species_scca.csv",  quote=F, row.names=F) #Make sure it's the species one

spec.mat <- as.matrix(spec)
v <- spec.mat %*% betaS[,2]
spec.deflated <- spec.mat - v %*% (t(v) %*% spec.mat)/(sum(v*v))
spec.deflated <- as.data.frame(spec.deflated)
write.table(spec.deflated, "/workdir/users/fnn3/twins_uk/scca/species/twins240_deflatedSpecies_scca.csv", quote=F, sep="\t", row.names=F) 
